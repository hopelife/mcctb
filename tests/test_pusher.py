...
class TestData(unittest.TestCase):
    ...
    #     attachments_dict['pretext'] = "(테스트)attachments 블록 전에 나타나는 text"
    #     attachments_dict['title'] = "(테스트)다른 텍스트 보다 크고 볼드되어서 보이는 title"
    #     attachments_dict['title_link'] = "https://corikachu.github.io"
    #     attachments_dict['fallback'] = "(테스트)클라이언트에서 노티피케이션에 보이는 텍스트 입니다. attachment 블록에는 나타나지 않습니다"
    #     attachments_dict['text'] = "(테스트)본문 텍스트! 5줄이 넘어가면 *show more*로 보이게 됩니다."
    ...
    def test_telegram_send_message(self):
        print(inspect.stack()[0][3])
        self.telegram.send_message(sessionname='1234', username='moon_dev_bot', message='telegram test from PC')
        #self.slack.send_message("#trading-bot", 'test message in unittest')
